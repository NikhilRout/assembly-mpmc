/*
program for finding largest number in an array of registers
length of array -> d:20H
array starts from -> d:30H
largest number found -> d:40H
*/
...
ORG 0000H
	...
	LOOP:
		...
		CJNE A, B, LARGER ;if A != B, jump to LARGER
		JMP NOTLARGER
		LARGER:
			JC NOTLARGER ;if carry is generated, A<B jump to NOTSMALLER
			MOV B, A ;since carry is not generated, A>B
			         ;Move the value of A into B, so that B remains the largest
		NOTLARGER:
			INC R0 ;A =< B, nothing has to be done, check next
	DJNZ R1, LOOP ;repeat this comparison loop until R1 = 0
	MOV 40H, B ;largest number moved to display
END